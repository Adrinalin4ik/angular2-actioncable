...
export class Channel {
  ...
  constructor(public cable: Cable, public name: string, public params = {}) {
    ...
    this.baseChannel = this.cable.baseCable.subscriptions.create(channelParams, {
      received: (data: any) => { this.messages = Observable.create(observer => {observer.next(data}) },
      connected: (data: any) => { this.onconnected = Observable.create(observer => {observer.next(data}) },
      open: (data: any) => { this.onopen = Observable.create(observer => {observer.next(data}) },
      close: (data: any) => { this.onclose = Observable.create(observer => {observer.next(data}) },
      error: (data: any) => { this.onerror = Observable.create(observer => {observer.next(data}) }
    });
